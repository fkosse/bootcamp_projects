...
class MultiTrader(tpqoa.tpqoa):
    def __init__(self, conf_file, instrument, bar_length, SMA_S, SMA_L, SMA, dev, units):
        ...
        #*****************add 1st strategy-specific attributes here******************
    ...
    def on_success(self, time, bid, ask):
        ...
        recent_tick = pd.to_datetime(time)
           ...
           # define stop
        if recent_tick.time() >= pd.to_datetime("12:00").time():
    ...
    def define_strategy(self): # "strategy-specific"
        ...
        df["position_MR"] = np.where(df[self.instrument] < df.Lower, 1, np.nan)
        df["position_MR"] = np.where(df[self.instrument] > df.Upper, -1, df["position"])
        df["position_MR"] = np.where(df.distance * df.distance.shift(1) < 0, 0, df["position"])
        df["position_MR"] = df.position.ffill().fillna(0)
        ...
        df["position"] = np.sign(df.position_MR + df.position_SMA)
        #***********************************************************************
...
if __name__ == "__main__":
    ...
    trader = MultiTrader(r"C:\Users\Administrator\Desktop\Oanda\oanda.cfg", "EUR_USD", "20min", SMA_S, SMA_L, SMA, dev, units = 100000)
